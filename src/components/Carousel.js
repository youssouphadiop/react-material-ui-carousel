...
class Carousel extends Component {
  ...
  setActive(index, callback = () => {}, runCallbacks = true) {
    const { onChange, timeout, children, strictIndexing } = sanitizeProps(
      this.props
    );
  ...
  next(event) {
    ...
    const nextActive =
      this.state.active + 1 > children.length - 1 ? 0 : this.state.active + 1;
  ...
  prev(event) {
    ...
    const nextActive =
      this.state.active - 1 < 0 ? children.length - 1 : this.state.active - 1;
      ...
      >
        ...
        {!navButtonsAlwaysInvisible && (
          ...
          <Forward onClick={this.next}>
            {" "}
            <StyledPlayArrowIcon htmlColor="#757575" />{" "}
          </Forward>
        )}
  ...
  for (let i = 0; i < props.length; i++) {
    ...
    let className =
      props.indicatorProps !== undefined
        ...
        : undefined;
    const activeStyle =
      props.activeIndicatorProps !== undefined
        ? props.activeIndicatorProps.style
        : undefined;
    const activeClassName =
      props.activeIndicatorProps !== undefined
        ? props.activeIndicatorProps.className
        : undefined;
...
`;
const Backward = styled(CustomButton)`
  left: -11.5%;
  top: -12%;
`;
const Forward = styled(CustomButton)`
  right: -11.5%;
  top: -12%;
`;
const FastRewind = styled(CustomButton)`
  left: -5.8%;
  top: -12%;
`;
const FastForward = styled(CustomButton)`
  right: -5.8%;
  top: -12%;
`;
